  ...
  return (
    <>
      ...
      <AppearingOnScroll anchor='anchor-header'>
        {(isShow: any) => (
          <Header
            onClickMobileMenu={handleToggleMenu}
            className={`${isShow ? 'mt-0' : '-mt-100'} ${
              openMobileMenu ? '' : 'header-shadow'
            } header-sticky`}
          />
        )}
      </AppearingOnScroll>
      <div
        ...
        } mobile-menu container mx-auto bg-white`}
      >
        <ul className=''>
          {menuItems.map((item, index) => (
            <li onClick={handleClose} key={index} style={{ opacity: 1 }}>
              <Link
                className='block whitespace-nowrap py-2 text-base font-medium text-gray-800 hover:text-blue-600'
                href={item.href}
              >
                {item.label}
              </Link>
